...
class double_text_align(nn.Module):
    ...
    def forward(self,ff_start,ff_end,lf_start,lf_end,ff_action_label,lf_action_label,text_description):
        ...
        loss_lf = F.cross_entropy(lf_prediction, ff_action_label) # do not need one hot in cross-entropy

        ###============== language contrastive loss ===================###
        all_steps = self.tokenizer(Crosstask_steps, padding='max_length', truncation=True, max_length=35, return_tensors="pt").to(ff_start.device)
        step_output = self.text_encoder(all_steps.input_ids, 
                                   attention_mask = all_steps.attention_mask,        
                                   return_dict = True,
                                   mode = 'text'
                                  )
        step_feat = F.normalize(self.text_proj(step_output.last_hidden_state[:,0,:]),dim=-1)
