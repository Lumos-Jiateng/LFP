...
import numpy as np
import os
from data.coin30_custom import step_list

language_sup_path = '/nfs4-p1/ljt/github/PTU/Double_retrieval/language_sup/coin_features.npy'
...
class Coin_align(nn.Module):
    def __init__(self,                 
        ...
        # new language head here for the middle step
        self.cls_head_for_mf = nn.Sequential(
                  nn.Linear(self.text_encoder.config.hidden_size, self.text_encoder.config.hidden_size),
                  nn.ReLU(),
                  nn.Linear(self.text_encoder.config.hidden_size, class_number)
                )   
        self.language_head_for_mf = nn.Sequential(
                  nn.Linear(self.text_encoder.config.hidden_size, self.text_encoder.config.hidden_size),
                  nn.ReLU(),
                  nn.Linear(self.text_encoder.config.hidden_size, self.predict_visual_dim)
                )
    ...
    def forward(self,text_description,ff_start,lf_end,ff_action_label,lf_action_label,mf_action_label):
        ...
        # add in mf head
        mf_prediction = self.cls_head_for_mf(hidden_state)
        mf_embedding = self.language_head_for_mf(hidden_state)
        
        ###============== classification loss ===================###

        #change to one-hot targets
        ff_target = F.one_hot(ff_action_label, num_classes=118) #[1,3] -> [1,0,0...],[0,0,1...]
        lf_target = F.one_hot(lf_action_label, num_classes=118) 
        ...
        mf_target = F.one_hot(mf_action_label,num_classes=118)
        loss_mf = F.cross_entropy(mf_prediction,mf_action_label)
        ...
        mf_sim_matrix = mf_embedding @ step_feat.t()
        language_loss_mf= -torch.sum(F.log_softmax(mf_sim_matrix,dim=1)*mf_target,dim=1).mean()
        
        return loss_ff , loss_lf , language_loss_ff , language_loss_lf ,loss_mf, language_loss_mf
    
def load_coin_align_with_blip(pretrained='',**kwargs):
...
if __name__ == '__main__':
    ...
    model = load_coin_align_with_blip(pretrained = url)
